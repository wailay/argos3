/**
 * @file <argos3/plugins/robots/foot-bot/control_interface/ci_spiri_proximity_sensor.cpp>
 *
 * @author Carlo Pinciroli <ilpincy@gmail.com>
 * @author Pierre-Yves Lajoie <lajoie.py@gmail.com>
 */

#include "ci_spiri_proximity_sensor.h"
...
namespace argos {
   ...
   static CRadians SPACING = CRadians(ARGOS_PI / 2.0f);
   static CRadians START_ANGLE = SPACING * 0.5f;

   /****************************************/
   /****************************************/

   CCI_SpiriProximitySensor::CCI_SpiriProximitySensor() :
      m_tReadings(4) {
   ...
   const CCI_SpiriProximitySensor::TReadings& CCI_SpiriProximitySensor::GetReadings() const {
     return m_tReadings;
   }
...
#ifdef ARGOS_WITH_LUA
   void CCI_SpiriProximitySensor::CreateLuaState(lua_State* pt_lua_state) {
...
#ifdef ARGOS_WITH_LUA
   void CCI_SpiriProximitySensor::ReadingsToLuaState(lua_State* pt_lua_state) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_SpiriProximitySensor::SReading& s_reading) {
   ...
   std::ostream& operator<<(std::ostream& c_os,
                            const CCI_SpiriProximitySensor::TReadings& t_readings) {
      if(! t_readings.empty()) {
